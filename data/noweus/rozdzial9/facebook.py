...
class fbsession:
  ...
  def makedataset(self):
    ...
    # uzyskanie wszystkich informacji dotyczących wszystkich znajomych użytkownika
    friends=self.getfriends()
    info=self.getinfo(friends)
    ids1,ids2=[],[]
    rows=[]

    # zagnieżdżona pętla sprawdzająca każdą parę znajomych
    for i in range(len(friends)):
      f1=friends[i]
      data1=info[f1]
      
      # rozpoczęcie od i+1 w celu uniknięcia dzielenia
      for j in range(i+1,len(friends)):
        ...
        # generowanie kilku liczb na podstawie danych
        ...
        rows.append(row)
    # wywoływanie listy arefriends w blokach dla każdej pary osób
    arefriends=[]
